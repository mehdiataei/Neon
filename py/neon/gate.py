...
class Gate(object):
    def __init__(self):
        ...
        lib_path = os.path.dirname(current_file_path) + "/../../cmake-build-release/libNeonPy/liblibNeonPy.so"
...
#         ## new_field
#         self.neon.lib.dGrid_dField_new.argtypes = [self.handle_type,
#                                                       self.handle_type]
#         ## delete_field
#         self.neon.lib.dGrid_dField_delete.argtypes = [self.handle_type]
#
#     def help_new(self):
#         if self.handle == 0:
#             raise Exception('DGrid: Invalid handle')
#
#         res = self.neon.lib.dGrid_dField_new(self.handle)
#         if res != 0:
...
#         if self.handle == 0:
#             return
#         res = self.neon.lib.dGrid_dField_delete(self.handle)
#         if res != 0:
...
#
#         # grid_new
#         self.neon.lib.dGrid_new.argtypes = [self.neon.handle_type]
#         # self.lib.grid_new.re = [ctypes.c_int]
#
#         # grid_delete
#         self.neon.lib.dGrid_delete.argtypes = [self.neon.handle_type]
#         # self.lib.grid_delete.restype = [ctypes.c_int]
#
#         self.neon.lib.dGrid_get_span.argtypes = [self.neon.handle_type,
...
#             raise Exception('DGrid: Invalid handle')
#
#         res = self.neon.lib.dGrid_new(self.handle)
#         if res != 0:
...
#         if self.handle == 0:
#             return
#         res = self.neon.lib.dGrid_delete(self.handle)
#         if res != 0:
#             raise Exception('Failed to delete grid')
#
#     def new_field(self) -> DField:
#         field = DField(self.neon, self.handle)
#         return field
#
#     def get_span(self,
#                  execution: Execution,
...
#         ex:int = execution
#
#         res = self.neon.lib.dGrid_get_span(self.handle, span, ex, set_idx, data_view)
#         if res != 0:
