...
class Report(object):
    ...
    def help_load_api(self):
        ...
        def register_foo(name, argtypes, restype):
            foo = getattr(self.py_neon.lib, name, None)
        ...
        register_foo('report_new',
                     [ctypes.POINTER(self.py_neon.handle_type), ctypes.c_char_p],
                     ctypes.c_int)
        # ------------------------------------------------------------------
    ...
    def help_new(self):
        ret = self.py_neon.lib.report_new(self.handle, self.report_name)
        if ret != 0:
            # raise excetion
